...
class CalculadoraCientificaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btnPotencia).setOnClickListener { operacionPresionada("^") }
        ...
        // Configurar listeners para operaciones científicas
        findViewById<Button>(R.id.btnSin).setOnClickListener { funcionCientifica("sin", ::sin) }
        findViewById<Button>(R.id.btnCos).setOnClickListener { funcionCientifica("cos", ::cos) }
        findViewById<Button>(R.id.btnTan).setOnClickListener { funcionCientifica("tan", ::tan) }
        findViewById<Button>(R.id.btnLn).setOnClickListener { funcionCientifica("ln", ::ln) }
        findViewById<Button>(R.id.btnLog).setOnClickListener { funcionCientifica("log", ::log10) }
        findViewById<Button>(R.id.btnRaiz).setOnClickListener { funcionCientifica("√", ::sqrt) }
        findViewById<Button>(R.id.btnPi).setOnClickListener { insertarPi() }
    }
    ...
    private fun calcularResultado() {
        try {
            ...
            val resultado = when (operador) {
                ...
                "×" -> primerNumero * segundoNumero
                "÷" -> primerNumero / segundoNumero
                "^" -> primerNumero.pow(segundoNumero)
                else -> segundoNumero
            }
    ...
    private fun funcionCientifica(nombre: String, funcion: (Double) -> Double) {
        try {
            val numero = pantalla.text.toString().toDouble()
            val resultado = if (nombre in listOf("sin", "cos", "tan")) {
                funcion(numero * PI / 180) // Convertir a radianes
            } else {
                funcion(numero)
            }
            operacionCompleta = "$nombre($numero) = "
            operacion.text = operacionCompleta
            pantalla.text = String.format("%.8f", resultado).trimEnd('0').trimEnd('.')
            nuevoNumero = true
        } catch (e: Exception) {
            pantalla.text = "Error"
            operacion.text = ""
    ...
    private fun insertarPi() {
        pantalla.text = PI.toString()
        operacionCompleta += "π"
        operacion.text = operacionCompleta
        nuevoNumero = true
    }
